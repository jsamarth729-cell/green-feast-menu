...
| `bowls.html` / `bowls.css` / `bowls.js` | Screen 1 — Power Bowls: hero slideshow + 2×5 tile grid. The hero rotation also carries an **offers slide** twice per cycle (after the midpoint bowl and after the last one, `Math.floor(featured.length/2)` — not hardcoded indices), showing the two bowl add-ons at poster size with a longer 15s dwell against the bowls' 10s. Content in `data/config.json`'s `offersSlide`; markup/state in `bowls.js` (`offersSlideHTML()`, `slideDurations`, `queueNextSlide()`). Loads `core.css`/`core.js` first. See DESIGN-PRINCIPLES §3 for why an offers slide is a deliberate exception to "the hero sells one item at a time". |
...
| `beverages.html` / `beverages.css` / `beverages.js` | Screen 3 — Beverages: no hero/slideshow, no 525px dark left panel, and no dark header band — `.main-content` starts at the screen's top edge. Two side-by-side columns: a 3x2 Functional Smoothies grid and a 2x2 Coffee grid. The coffee column carries a second mode — a 764px `.offers-panel` that fades in over it on a 30s/15s cycle, showing the smoothie add-ons at poster size: headline, a no-added-sugar claim chip, a scoop photo, a supplier credit, then the two add-on price rows (content in `data/screen3-config.json`'s `offersPanel`). Loads `core.css`/`core.js` first. See DESIGN-PRINCIPLES §2 for the header-band history and the offers-panel's dark-anchor tradeoff. |
| `rmain.html` / `rmenu.html` / `rwrap.html` | **Temporary event takeover pages** (Ritual x Green Feast). One full-bleed image or video each, no `core.*`, no JS — so they are exempt from the `?v=N` bump rule. `rmain` = Screen 1's TV, `rmenu` = Screen 3's, `rwrap` = Screen 2's (video). Swapped in by pointing a TV's kiosk URL at them; the real boards are never modified. Assets in `images/events/`, built by `build-event-art.mjs`. **See `EVENT-SCREENS-PLAN.md`** for cutover/revert steps and every design decision — it is the reference, not this table. |
| `build-event-art.mjs` | Event-asset pipeline: source art -> 1920x1080 `images/events/*`, and the `rwrap` video trim. Offline tooling. Needs `ffmpeg-static` (devDependency, offline only). |
...
| `data/config.json` | Screen 1 static content: Build-Your-Own tile, `upsellHeading` + `upsell` rail items, and `offersSlide` (`{title, eyebrow, image, amount, amountNote, items}`) for the hero's offers slide. `offersSlide.items` intentionally stays two lines matching `upsell` — one per add-on. `offersSlide.image` (blank until an `images/nobg/extras-grid.png` 2×2 grid is generated and dropped in — see `SCREEN1-OFFERS-SLIDE-PLAN.md` §5/§8) will show four ingredients even though there are only two add-ons: paneer illustrates the protein add-on, the other three illustrate the premiums add-on. That is not stale copy needing a third `items` line; do not add one. |
...
| `data/screen3-config.json` | Screen 3 static content: the two section headings ("Functional Smoothies", "Coffee") and the upsell rail. `upsellHeading` (string) + `upsell` (array of pipe-delimited `"text \| price"` strings) — same shape as Screens 1–2, rendered through the shared `renderPipeUpsell()` in `core.js`. The smoothie section's sweetener disclaimer lives on `sections.smoothie.note` (rendered inline next to the "Functional Smoothies" heading), not on `upsell` — it's scoped to that section, not the rail. Screen 3's footer is now the same glossary block as Screens 1–2 (no board-specific footer content remains). `offersPanel` (`{title, chip, sweetener, eyebrow, image, credit, items:[{amount,what,price}]}`) is the content for the offers-panel second mode — a separate, deliberate content set from `upsell`; the rail keeps its own two add-ons unchanged. `credit` may carry a `<b>` tag (e.g. naming a supplier) and is injected via `innerHTML`, not `textContent` — every other field is plain text. |
...
| `images/nobg/` | Screen 1's transparent cut-outs: `<slug>-side.png` (hero) + `<slug>-top.png` (tile), plus `extras-grid.png` — the offers slide's feature art (solo image, no shared canvas; see DESIGN-PRINCIPLES §6). |
...
| `images/screen3/` | Screen 3's transparent cut-outs: one per smoothie/coffee (`<slug>.png`), plus `protein-scoop.png` — the offers panel's feature art. All trimmed to alpha bounding box only — **no shared canvas** (a deliberate departure from the Screen 2 pattern; see DESIGN-PRINCIPLES §6), though the ten drinks and the scoop are exempt from rule 2 for two different reasons (uniform cup set; solo feature art with nothing to normalize against). The board normalizes the drinks by CSS height; the scoop renders at a fixed 250px height in the offers panel. |
...
| `build-screen3-cutouts.mjs` | Screen 3's image pipeline. Trims pre-background-removed PNGs to their alpha bounding box only — no shared canvas. Two groups share this path for different reasons: the ten drinks are all the same cup, angle and lighting (a uniform set); `protein-scoop` is solo feature art in its own layout slot with nothing to normalize against (same exemption as Screen 2's `panini-hero`/`bbq-plate`). Source art lives in the local, gitignored `Screen3/` folder — this script reads from there. |
